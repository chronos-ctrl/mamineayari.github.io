...
I developed a **3D pose estimation solution** capable of locating and estimating the pose of **more than 150 different fixing types**, each defined by a **unique 3D geometry**. Rather than relying on classification or 2D appearance, the solution is based on **pure 3D geometric matching**.
...
For each keypoint:
1. A local neighborhood is defined.
...
- encoding the distribution of surface orientations around the keypoint.
